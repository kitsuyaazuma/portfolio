      ...
      <ul className="timeline timeline-vertical timeline-compact mx-auto max-w-2xl">
        {items.map((item, index) => (
          <li key={index}>
            ...
            <div
              className={`timeline-middle ${item.isCurrent ? "text-primary" : ""}`}
            >
              {getCategoryIcon(item.category)}
            </div>

            <div
              className={`timeline-end mb-6 w-full ${item.isCurrent ? "aura text-primary" : ""}`}
            >
              <div className="timeline-box w-full bg-base-100 text-base-content shadow-sm">
